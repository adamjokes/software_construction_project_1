...
import 'package:workampus/screens/jobProviderApp/contractsTab/completed/completedContractDetail_view.dart';
...
class CompletedContractWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // determine status
    Text _status;
    if (!completedContract.isReviewedByJP && completedContract.isReviewedByJS) {
      _status = Text(
        ' - review now!',
        style: TextStyle(
            color: accentColor, fontSize: 13.0, fontWeight: FontWeight.w400),
      );
    } else if (!completedContract.isReviewedByJP &&
        !completedContract.isReviewedByJS) {
      _status = Text(
        ' - review now',
        style: TextStyle(
